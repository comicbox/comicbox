...
begin
    update 
        user_book
    set 
        current_page = min(current_page, (select page_count from book where id = user_book.book_id) - 1),
        last_page_read = min(max(last_page_read, current_page), (select page_count from book where id = user_book.book_id) - 1)
    where  
